#!/usr/bin/env python
"""
Search for BDD100K validation images with very high coverage (90-100%).
...
def evaluate_image_coverage(image_path, predictor, calibrated_model, bdd_annotations, bdd_coco_mapping, score_threshold=0.3):
    ...
    img_id = None
    for img_info in bdd_annotations['images']:
    ...
    gt_boxes = []
    for ann in bdd_annotations['annotations']:
...
def main():
    ...
    print(f"Loading BDD100K annotations from: {bdd_ann_file}")
    ...
    val_dir = "/ssd_4TB/divake/conformal-od/data/bdd100k/images/100k/val"
    ...
    for img_path in tqdm(image_files, desc="Evaluating images"):
        result = evaluate_image_coverage(
            ...
            bdd_annotations, bdd_coco_mapping
        )
    ...
    # Save results
    output_file = "high_coverage_images_bdd100k_cqr.json"
    ...
    if high_coverage_images:
        ...
        print(f"{'Rank':<5} {'Image':<30} {'Coverage':<10} {'Covered/Total':<15} {'Size':<15}")
        print("-" * 90)
        
        for i, img in enumerate(high_coverage_images[:10]):
            print(f"{i+1:<5} {img['image_name']:<30} {img['coverage_rate']*100:>6.1f}% "
